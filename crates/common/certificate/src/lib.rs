            ...
            Err(err) => {
                ...
                if e.contains("Failed to parse the received frame") {
                    ...
                    let Some(current_cert) = current_cert else {
                        return Err(CertificateError::Other(anyhow::anyhow!("tedge-p11-server can only renew existing certificates but there's no existing certificate; upgrade tedge-p11-server or generate a self-signed certificate first")));
                    };
